...
1. **GitHub Pages 部署源**：你的 repo 当前应该是从 `main` 分支根目录部署的（因为 `chess/` 和 `religion/` 是直接放在根目录的）。我们的方案沿用同样的部署源，把 `watchlist/` 也放在根目录。如果你不确定，可以去 Settings → Pages 检查一下："Build and deployment" 下的 Source 应该是 "Deploy from a branch"，Branch 是 `main`，Folder 是 `/ (root)`。
...
先把这个 bundle 解压。然后在你本地的 `Website` repo 根目录下，**只复制这三个新增的部分**进去：
...
### 问题 1：Workflow 失败，提示 "Permission denied"（推送回 repo 时）

去 Settings → Actions → General → Workflow permissions，确保选了 "Read and write permissions"。这是最常见的第一次设置时的疏漏。
